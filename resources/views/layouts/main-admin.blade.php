					...
					@if (Auth::user()->notifications()) 
					 @foreach (Auth::user()->unreadNotifications as $notification)
						@if (isset($notification->data['neworder']))
							...
							<a href="{{url('newOrderView/'.$notification->id.'/'.$notification->data['neworder']['id'])}}" class="dropdown-item">
							...
							<p class="text-gray ellipsis mb-0">&nbsp;&nbsp;{{$notification->data['neworder']['orderid']}}</p>
							<div class="dropdown-divider"></div>
						@endif
						@if (isset($notification->data['canceleOrder']))
							<a  href="{{url('newOrderView/'.$notification->id.'/'.$notification->data['canceleOrder']['id'])}}" class="dropdown-item">
								<i class="fas fa-envelope mr-2"></i>{{$notification->data['canceleOrder']['title']}}
								<span class="float-right text-muted text-sm">3 mins</span>
							</a>
							<p class="text-gray ellipsis mb-0">&nbsp;&nbsp;{{$notification->data['canceleOrder']['orderid']}}</p>
							<div class="dropdown-divider"></div>
						@endif
						@if (isset($notification->data['orderComplaint']))
						{{-- href="{{url('orderNotificationview/'.$notification->id.'/'.$notification->data['neworder']['id'])}}"  --}}

							<a href="{{url('complaintviewN/'.$notification->id.'/'.$notification->data['orderComplaint']['id'])}}" class="dropdown-item">
			...
			<ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
				...
				@can('manage-orders')
					<li class="nav-item {{ (request()->is('assign-orders')) ? 'menu-open' : '' }}">
						<a href="{{ route('assign-orders') }}" class="nav-link {{ (request()->is('assign-orders')) ? 'active' : '' }}">
				...
				<li class="nav-item {{ (request()->is('assigned-orders')) ? 'menu-open' : '' }}">
					<a href="{{ route('assigned-orders') }}" class="nav-link {{ (request()->is('assigned-orders')) ? 'active' : '' }}">
					<i class="fa fa-circle nav-icon"></i>
					<p>
						Assigned Order
					</p>
				...
				<li class="nav-item {{ (request()->is('pickup-orders')) ? 'menu-open' : '' }}">
					<a href="{{ route('pickup-orders') }}" class="nav-link {{ (request()->is('pickup-orders')) ? 'active' : '' }}">
					<i class="fa fa-circle nav-icon"></i>
					<p>
						Pickup Order
					</p>
				...
				<li class="nav-item {{ (request()->is('history-orders')) ? 'menu-open' : '' }}">
					<a href="{{ route('history-orders') }}" class="nav-link {{ (request()->is('history-orders')) ? 'active' : '' }}">
					<i class="fa fa-circle nav-icon"></i>
					<p>
						Order History
					</p>
				...
				<li class="nav-item {{ (request()->is('complaint-orders')) ? 'menu-open' : '' }}">
					<a href="{{ route('complaint-orders') }}" class="nav-link {{ (request()->is('complaint-orders')) ? 'active' : '' }}">
				...
				@if(auth()->user()->can('coupon') || auth()->user()->can('new-coupon'))
					<li class="nav-item has-treeview {{ (request()->is('coupon') || request()->is('new-coupon')) ? 'menu-open' : '' }}">
						...
						<ul class="nav nav-treeview">
							@can('coupon')
								<li class="nav-item">
									<a href="{{ route('coupon') }}" class="nav-link {{ (request()->is('coupon')) ? 'active' : '' }}">
										<i class="fa fa-gift" aria-hidden="true"></i>
										<p> All Coupn </p>
									</a>
								</li>
							...
							@can('new-coupon')
								<li class="nav-item">
									<a href="{{ route('create-coupon') }}" class="nav-link {{ (request()->is('new-coupon')) ? 'active' : '' }}">
										<i class="fa fa-gift" aria-hidden="true"></i>
										<p>Add Coupon </p>
									</a>
								</li>
				...
				@if(auth()->user()->can('riders') || auth()->user()->can('new-rider'))
					<li class="nav-item has-treeview {{ (request()->is('riders') || request()->is('new-rider')) ? 'menu-open' : '' }}">
						...
						<ul class="nav nav-treeview">
							...
							@can('new-rider')
								<li class="nav-item">
									<a href="{{ route('new-rider') }}" class="nav-link {{ (request()->is('new-rider')) ? 'active' : '' }}">
										<i class="fa fa fa-motorcycle nav-icon"></i>
										<p>Add Rider </p>
									</a>
								</li>
				...
				@if(auth()->user()->can('home-delivery') || auth()->user()->can('pickup'))
					<li class="nav-item has-treeview {{ (request()->is('pickup') || request()->is('home-delivery')) ? 'menu-open' : '' }}">
						...
						<ul class="nav nav-treeview">
							@can('home-delivery')
								<li class="nav-item">
									<a href="{{ route('home-delivery') }}" class="nav-link {{ (request()->is('home-delivery')) ? 'active' : '' }}">
										<i class="far fa-circle nav-icon"></i>
										<p> Home Delivery </p>
									</a>
								</li>
							...
							@can('pickup')
								<li class="nav-item">
									<a href="{{ route('pickup') }}" class="nav-link {{ (request()->is('pickup')) ? 'active' : '' }}">
										<i class="far fa-circle nav-icon"></i>
										<p> Take Away </p>
									</a>
								</li>
