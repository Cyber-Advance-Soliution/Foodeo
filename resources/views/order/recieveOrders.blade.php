		...
		<div class="modal-dialog">
		  <div class="modal-content">
			<div class="modal-header">
			  <h4 class="modal-title"> Editttt Order Status </h4>
				...
				<form action="{{ route('update-order') }}" method="post" role="form">
					...
					<!--<div class="form-group">
						...
						<select class="form-control" id="employee_id" name="employee_id" style="width: 100%;">
							...
							@foreach($modelEmployee as $employee)
					...
					<div class="form-group">
						<label> Status </label>
						<select class="form-control" required id="order-status" name="order_status" style="width: 100%;">
							<option selected="selected"> -- Select -- </option>
						</select>
					</div>
          ...
          <div class="card">
            <div class="card-header">
				...
				<div class="card-tools">
                	<a href="{{ route('manage-orders') }}" class="btn btn-success btn-sm"> <i class="fa fa-tasks nav-icon"></i> Manage Orders </a>
				</div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="recieved-orders" class="table table-bordered table-hover">
                ...
                <tbody>
					...
					<tr id="{{ $key }}">
						...
						<td>  
							<a href="javascript://" onclick="statusList({{ $order->id }})" id="{{ $key }}" class="btn btn-block btn-success btn-flat">  <i class="fa fa-check"></i> Accept </a>						
							<a href="javascript://" onclick="updateOrder({{ $order->id }}, 6, this.id)" id="{{ $key }}" class="btn btn-block btn-danger btn-flat">  <i class="fa fa-times"></i> Reject </a>						
						</td>
					</tr>
					@endforeach
                </tbody>
