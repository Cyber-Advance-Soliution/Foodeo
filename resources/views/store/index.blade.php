            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <a href="{{ route('new-store') }}" class="btn btn-success btn-sm"> <i class="fa fa-plus"></i>
                            Add Store </a>
                    </div>
                </div>
                <!-- /.card-header -->
                    ...
                    <table id="stores" class="table table-bordered table-hover table-responsive">
                        ...
                        @foreach($model as $key => $store)
                            <tr>
                                <td> {{ ++$i }} </td>
                                <td>
                                    <a href="{{ route('store-view', ['id' => $store->id]) }}"> {{ $store->store_name }} </a>
                                </td>
                                <td> {{ $store->status }} </td>
                                ...
                                <td class="action-width">
                                    <a href="{{ route('update-status', ['id' => $store->id]) }}"
                                       class="btn btn-block btn-{{ ($store->visible_status == 1) ? 'danger' : 'success' }} btn-flat">
                                        <i class="fa {{ ($store->visible_status == 1) ? 'fa-eye-slash' : 'fa-eye' }}"></i> {{ ($store->visible_status == 1) ? 'Hide' : 'Visible' }}
                                    </a>
