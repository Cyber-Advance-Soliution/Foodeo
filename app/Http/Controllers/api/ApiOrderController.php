...
class ApiOrderController extends Controller
{
    ...
    public function saveOrder(Request $request)
    {
        ...
        if($orderId){
            ...
            $customer = Customer::where('u_id', $user['u_id'])->first();
//            dd($customer);
//            return response()->json(['message'=>'testing']);
            Notification::send($customer, new OrderNotification($neworder, $canceleOrder, $orderComplaint));
        }
    ...
    public function allOrder(Request $request)
    {
        ...
        foreach($orderdata as $item){
            ...
            $orderList[$i]['order_products']=$item->orderProducts;
        ...
        // echo '<pre>'.print_r($orderList, true).'</pre>';
    ...
    public function orderCancel(Request $request)
    {
        ...
        if($req){
            if($req->status==0 ){
                ...
                $orderid = ' Order number is ' .$req->id;
                $canceleorder= collect(['title' => $title, 'orderid' => $orderid, 'id' => $req->id]);
                Notification::send($admin, new OrderNotification($neworder, $canceleorder, $orderComplaint));
            }else{
                $response = [
                    'Message' => 'fail,order already canceled',
                    'Status' => 0,
                    'Data' => null
                ];
            }
            ...
            $response = [
                'Message' => 'fail,detail does not match',
                'Status' => 0,
                'Data' => null
